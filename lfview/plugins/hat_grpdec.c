/*
 * hat_grp.c
 * decode GRP format images in the `Hatsune no Naisho!'.
 * 1998/03/12 by TF
 * 1999/05/08 updated
 */
#include <stdio.h>
...
GRP *read_grp(const char *filename)
{
  return NULL;
}
...
GRP *read_grp_from_data(unsigned char *palette, int psize, unsigned char *body, int bsize)
{
  GRP *grp;
...
static void get_plain_palette(GRP *grp, unsigned char *palette, int psize, int idx)
{
  ...
  for (i = 0; i < grp->color_num; i++) {
    ...
    if (grp->color_num < 16) {
      /* should we change a palette on 16 colors? */
      grp->palette[id][GRP_B] = (grp->palette[id][GRP_R] & 0xf0) + (grp->palette[id][GRP_R] >> 4);
      grp->palette[id][GRP_G] = (grp->palette[id][GRP_G] & 0xf0) + (grp->palette[id][GRP_G] >> 4);
      grp->palette[id][GRP_R] = (grp->palette[id][GRP_B] & 0xf0) + (grp->palette[id][GRP_B] >> 4);
    }
  }
}
